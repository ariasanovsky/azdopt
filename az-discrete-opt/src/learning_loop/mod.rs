use crate::{az_model::AzModel, int_min_tree::state_data::UpperEstimateData, space::StateActionSpace, path::ActionPathFor, state::cost::Cost};

use self::{state::StateData, tree::TreeData, prediction::PredictionData};

pub mod tree;
pub mod state;
pub mod prediction;

pub fn par_roll_out_episode<const BATCH: usize, const STATE: usize, const ACTION: usize, const GAIN: usize, Space, C, P>(
    ...
    C: Cost<f32> + Send + Sync,
{
    ...
    ends.par_update_existing_nodes::<Space, ACTION, GAIN>(states.get_costs(), states.get_states(), &predictions);
    trees.insert_nodes();
}