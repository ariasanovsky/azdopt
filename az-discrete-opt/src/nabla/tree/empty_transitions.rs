...
impl<P> SearchTree<P> {
        ...
        P: Ord + ActionPath + ActionPathFor<Space> + Clone,
    {
        ...
        while let Some((parent_pos, parent_decay)) = decay_tracker.pop_front() {
            ...
            children.retain(|Transition { state_position, action_position }| {
            ...
            for Transition { state_position: child_pos, action_position } in self.in_neighborhoods.get(parent_pos.get()).unwrap() {
